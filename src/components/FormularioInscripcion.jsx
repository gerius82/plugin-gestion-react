...
const FormularioInscripcion = () => {
  ...
  const TIPO_INSCRIPCION = "CICLO_2026";
  ...
  const confirmarEnvio = async () => {
    ...
    (async () => {
        ...
        const payload = {
          ...
          tipo_inscripcion: TIPO_INSCRIPCION,   // 👈 marca que es ciclo regular
        };
      ...
      {mensajeExito ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <h2 className="text-2xl font-bold text-center mb-6">Formulario de Inscripción</h2>
            ...
            <div className="mb-3 w-full p-4 rounded-lg bg-gray-50 shadow-sm">
                ...
                <p className="text-xs sm:text-sm text-gray-600 leading-tight">Por favor completá la información básica del estudiante.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
            <button
                onClick={(e) => {
                e.preventDefault();
                setFormulario(prev => ({ ...prev, curso: 'Robótica Básica' }));
                }}
                className={`relative p-4 rounded-lg border shadow transition text-left ${
                formulario?.curso === 'Robótica Básica'
                    ? 'border-green-300 bg-green-200 hover:bg-green-200'
                    : 'border-gray-200 hover:bg-gray-200 hover:shadow-md hover:scale-105'
                }`}
            >
                <img src="/img/robotica-basica.jpg" alt="Robótica Básica" className="w-full h-32 object-cover rounded" />
                <h4 className="text-lg font-semibold mt-2">Robótica Básica</h4>
                <p className="text-sm text-gray-600">Ideal para iniciar en robótica desde cero, con actividades prácticas.</p>
                <span className="text-xs text-green-700">Para todas las edades</span>
            </button>

            <button
                onClick={(e) => {
                e.preventDefault();
                setFormulario(prev => ({ ...prev, curso: 'Robótica Avanzada' }));
                }}
                className={`relative p-4 rounded-lg border shadow transition text-left ${
                formulario?.curso === 'Robótica Avanzada'
                    ? 'border-green-300 bg-green-200 hover:bg-green-200'
                    : 'border-gray-200 hover:bg-gray-200 hover:shadow-md hover:scale-105'
                }`}
            >
                <img src="/img/robotica-avanzada.jpg" alt="Robótica Avanzada" className="w-full h-32 object-cover rounded" />
                <h4 className="text-lg font-semibold mt-2">Robótica Avanzada</h4>
                <p className="text-sm text-gray-600">Pensada para quienes ya tienen conocimientos previos y buscan desafíos más complejos.</p>
                <span className="text-xs text-blue-700">Requisito: haber realizado Robótica Básica</span>
            </button>

            <button
                onClick={(e) => {
                e.preventDefault();
                setFormulario(prev => ({ ...prev, curso: 'Programación con Scratch' }));
                }}
                className={`relative p-4 rounded-lg border shadow transition text-left ${
                formulario?.curso === 'Programación con Scratch'
                    ? 'border-green-300 bg-green-200 hover:bg-green-200'
                    : 'border-gray-200 hover:bg-gray-200 hover:shadow-md hover:scale-105'
                }`}
            >
                <img src="/img/scratch.jpg" alt="Programación con Scratch" className="w-full h-32 object-cover rounded" />
                <h4 className="text-lg font-semibold mt-2">Programación con Scratch</h4>
                <p className="text-sm text-gray-600">Introduce a la programación de forma visual y divertida.</p>
                <span className="text-xs text-purple-700">Edad mínima 8 años</span>
            </button>

            <button
                disabled
                className="relative p-4 rounded-lg border border-gray-300 shadow bg-gray-100 text-gray-400 cursor-not-allowed text-left hover:bg-gray-200"
            >
                <img src="/img/arduino.jpg" alt="Programación y Robótica con Arduino" className="w-full h-32 object-cover rounded opacity-60" />
                <h4 className="text-lg font-semibold mt-2">Arduino</h4>
                <p className="text-sm text-gray-500">Temporalmente no disponible.</p>
                <span className="text-xs text-red-500">Próximamente</span>
            </button>
